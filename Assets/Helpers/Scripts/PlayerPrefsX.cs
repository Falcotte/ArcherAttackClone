...
public class PlayerPrefsX
{
    ...
    public static double GetDouble(string key, double defaultReturn = 0d)
    {
        return Double.Parse(PlayerPrefs.GetString(key, defaultReturn.ToString()));
    }
    ...
    public static void SetTimestamp(string timestampKey, DateTime time)
    {
        PlayerPrefs.SetString(timestampKey, time.ToFileTimeUtc().ToString());
    }

    public static DateTime GetTimestamp(string timestampKey)
    {
        var str = PlayerPrefs.GetString(timestampKey, DateTime.UtcNow.AddYears(-1).ToFileTimeUtc().ToString());
        return DateTime.FromFileTimeUtc(Convert.ToInt64(str));
    }
